 ...
 */
export function extractOverlapInfo(manifest: SkillManifest): SkillOverlapInfo {
 ...
 */
export function readAllManifests(skillsDir: string): SkillManifest[] {
...
export function generateMatrix(skillsDir: string): MatrixEntry[] {
  const manifests = readAllManifests(skillsDir);
  const overlapInfos = manifests.map(extractOverlapInfo);
