...
import { RESOLUTIONS_DIR, SHIPPED_RESOLUTIONS_DIR } from './constants.js';
import { ResolutionMeta } from './types.js';
 ...
 * Verifies input_hashes from meta.yaml match before loading.
 * Returns true if loaded successfully, false if not found or hashes don't match.
 */
export function loadResolutions(
  ...
  for (const { preimage, resolution } of pairs) {
...
export function saveResolution(
  skills: string[],
  files: { relPath: string; preimage: string; resolution: string }[],
